...
(function() {
  ...
  // === АВТОМАТИЧЕСКИЙ ПОИСК ПО НАЗВАНИЮ ФИЛЬМА ===
  function startAutoSearch(balancerItem, movieTitle) {
    // Очищаем название от лишнего (годы, сезоны и т.д.)
    ...
    var searchComponent = createSearchComponent(balancerItem, function(selectedItem, balancer) {
      ...
      showMoviePlayer(balancer, selectedItem);
    });
  ...
  function cleanMovieTitle(title) {
    ...
    // Удаляем год в скобках (2023), (2020-2023) и т.д.
    title = title.replace(/\s*\(\d{4}.*?\)/g, '');
    
    // Удаляем сезоны и серии
  ...
  // === ПОКАЗ ПЛЕЕРА ===
  function showMoviePlayer(balancerItem, movieItem) {
  ...
  // === ОСНОВНОЙ КОМПОНЕНТ ANDROZON (УПРОЩЕННЫЙ) ===
  function component(object) {
    ...
    this.initialize = function() {
      ...
      // Balancers list - СРАЗУ ЗАПУСКАЕМ ПОИСК ПРИ ВЫБОРЕ
      scroll.append($('<div style="padding:12px 16px;color:#ddd;font-weight:600;">Выберите источник для автоматического поиска</div>'));
      ...
      BALANCERS.servers.forEach((balancerItem) => {
        ...
        el.append(`<div><div style="font-weight:700;color:#fff;">🎬 ${balancerItem.title}</div><div style="font-size:12px;color:#bbb;">Автопоиск: "${cleanMovieTitle(movie.title || movie.name)}"</div></div>`);
        el.append('<div style="color:#fff;font-size:12px;">' + (balancerItem.weight > 1 ? '🔥' : '') + '</div>');
        
        el.on('hover:enter', function(){
          // СРАЗУ запускаем автоматический поиск по названию фильма
          startAutoSearch(balancerItem, movie.title || movie.name);
        });
      ...
      var manualSearch = $('<div class="selector" style="padding:12px;margin:16px;border-radius:8px;background:rgba(255,107,53,0.2);text-align:center;cursor:pointer;border:1px solid #FF6B35;"><div style="font-weight:600;color:#FF6B35;">✏️ Ручной поиск (ввести название)</div></div>');
      manualSearch.on('hover:enter', function(){
        // Запускаем пустой поиск для ручного ввода
        var searchComponent = createSearchComponent(BALANCERS.servers[0], function(selectedItem, balancer) {
          showMoviePlayer(balancer, selectedItem);
        });
  ...
  // === ИНИЦИАЛИЗАЦИЯ ===
  function init() {
    console.log('🎯 Androzon Auto Search v16 initialized');
