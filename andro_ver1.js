(function(){
    ...
    async function loadLinksFromBalancer(item, balancer){
        try {
            ...
            // простейший пример парсера, позже можно заменить на полноценный JSON-parser
            const links = [];
            const regex = /href="([^"]+)"/g;
            let m;
    ...
    async function showResults(list, query){
        ...
        // Подключаем событие выбора
        $('.content__list .selector').on('hover:enter', async function(){
            const idx = $(this).index();
            const selected = items[idx];
            let links = [];

            // Загружаем ссылки с активных балансеров
            ...
            const chosen = await Lampa.Modal.select('Выберите источник', links.map(l => ({title: l.title + ' [' + l.quality + ']', value: l.url})));
            if(chosen) Lampa.Player.play([{title: selected.title, url: chosen, subtitles: []}]);
        });
    }

    // ==================== Добавление кнопки в меню ====================
    Lampa.Listener.follow('activity', async function(e){
        if(e.type === 'ready' && e.name === 'main'){
            ...
            setTimeout(() => {
                ...
                const menu = root.find('.catalog__menu, .content__menu, .menu');
