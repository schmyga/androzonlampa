 ...
 * Переделка от z01.online/live (название изменено на SerVik)
 * Работает на всех версиях Lampa (yumata / mod / TV)
 */

(function () {
  const PLUGIN_ID = 'servik_button'; // Изменено с z01_button
  const PLUGIN_TITLE = 'SerVik'; // Изменено с Z01
  const DEBUG = true;

  // Балансеры с API (не трогаем)
  const BALANCERS = {
    'bwa': {
      name: 'BWA',
      url: 'https://bwa.to/rc',
  ...
  function log(...args) {
    if (DEBUG) console.log('[SerVik]', ...args); // Изменено в логах
  }

  function safeNoty(text) {
    try {
      if (window.Lampa?.Noty?.show) Lampa.Noty.show(text);
      else console.log('[SerVik]', text); // Изменено
  ...
  function createButton() {
    const button = $('<div class="full-start__button selector" style="background: linear-gradient(135deg, #ff7e00, #ffb700); border-radius: 50px; padding: 0 20px; margin-left: 10px;">')
      .html('<span>SerVik</span>'); // Изменено название в HTML

    button.on('hover:enter', function () {
      safeNoty('Загрузка через SerVik...'); // Изменено
      openSerVik(); // Функция переименована ниже
    });

    return button;
  }

  // Получить информацию о текущем фильме (не трогаем)
  ...
  // Поиск через BWA API (как в примерах) (не трогаем)
  ...
  // Универсальный поиск через прокси (не трогаем)
  ...
  // Поиск видео ссылок в HTML (не трогаем)
  ...
  // Показать найденные ссылки для выбора (не трогаем)
  ...
  // Парсинг и воспроизведение видео (не трогаем)
  ...
  // Извлечение видео URL из HTML (не трогаем)
  ...
  // Запуск видео в плеере Lampa (не трогаем)
  ...
  // Основное окно SerVik (переименовано с openZ01)
  function openSerVik() { // Переименовано
    const movieInfo = getMovieInfo();
    
    const html = $(`
      <div class="servik-modal"> <!-- Изменено класс -->
        <div style="padding: 1.5em;">
          <div style="text-align: center; margin-bottom: 1.5em;">
            <h2 style="margin: 0 0 10px 0; color: #ff7e00;">SerVik Player</h2> <!-- Изменено -->
  ...
  function insertButton() {
    ...
    if (buttonBlock.length && !buttonBlock.find('.' + PLUGIN_ID).length) {
      ...
      log('✅ Кнопка SerVik добавлена'); // Изменено
    }
  }
  ...
  function init() {
    followApp();
    log('✅ Плагин SerVik инициализирован'); // Изменено
  }
