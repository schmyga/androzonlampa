(function(){
    ...
    Lampa.Listener.follow('catalog', async function(e){
        if(e.type === 'complite'){
            await loadBalancers();
            const activity = Lampa.Activity.active();
            if(!activity || !activity.render) return;

            const menu = activity.render().find('.content__menu').first();
            ...
            console.log('✅ Кнопка Androzon добавлена в меню');
        }
    });

    console.log('✅ Плагин Androzon v1 (по логике v15) загружен');
})();
