(function() {
    ...
    if (isEnabled()) {
        Lampa.Listener.follow('full', function(e) {
            if (e.type === 'complite' && e.object.movie && e.object.movie.title) {
                ...
                let sources = BALANCERS.filter(b => b.active).sort((a,b) => b.weight - a.weight);
                function tryNext(i) {
                    if(i>=sources.length) return Lampa.Noty.show('Ничего не найдено');
                    createSearchModal(query, sources[i]);
                }
                tryNext(0);
            }
        });
            ...
            if(activity && activity.render){
                ...
                if(menu.length && !menu.find('.androzon-search').length){
                    ...
                    btn.on('hover:enter', function(){
                        let query = prompt('Введите название для поиска:');
                        if(query) createSearchModal(query, BALANCERS[0]);
                    });
                }
            }
        }
    ...
    console.log('Androzon v2.5 loaded - Settings > Plugins');
})();
