(function() {
    ...
    // Ручной поиск - добавление кнопки в главное меню (main) и каталог (catalog)
    // Исправлено: проверка на Lampa.Dom, fallback на jQuery, добавление в .full-start__menu или .catalog__menu
    Lampa.Listener.follow('main', function(e) {
        if (e.type === 'complite') {
            let activity = Lampa.Activity.active();
            if (activity && activity.render) {
                let renderEl = activity.render();
                let menuSelector = '.full-start__menu, .catalog__menu, .view--menu';
                let menu = renderEl.find(menuSelector);
                if (menu.length && !menu.find('.androzon-search').length) {
                    try {
                        let btn;
                        if (Lampa.Dom && Lampa.Dom.create) {
                            btn = Lampa.Dom.create('div', {
                                class: 'selector androzon-search',
                                html: '<div class="selector__ico">🔍</div><div class="selector__title">Androzon Поиск</div>'
                            });
                        } else {
                            btn = $('<div class="selector androzon-search"><div class="selector__ico">🔍</div><div class="selector__title">Androzon Поиск</div></div>');
                        }
                        menu.append(btn);
                        btn.on('hover:enter', function() {
                            // Fallback на стандартный Modal для ввода
                            Lampa.Modal.open({
                                title: 'Поиск',
                        ...
                        console.log('Androzon button added to main menu');
                    } catch (err) {
    ...
    // Дублируем для catalog, если main не сработает
    Lampa.Listener.follow('catalog', function(e) {
        if (e.type === 'complite') {
            let activity = Lampa.Activity.active();
            if (activity && activity.render) {
                let renderEl = activity.render();
                let menuSelector = '.catalog__menu, .view--menu';
                let menu = renderEl.find(menuSelector);
                if (menu.length && !menu.find('.androzon-search').length) {
                    try {
                        let btn;
                        ...
                        console.log('Androzon button added to catalog menu');
                    } catch (err) {
                        console.warn('Failed to add button to catalog:', err);
                    }
                }
            }
        }
    ...
    try {
        let originalPlugins = Lampa.Params.options && Lampa.Params.options.plugins;
    ...
    console.log('Androzon v2.6 - кнопка добавлена в main/catalog меню, проверь консоль');
})();
